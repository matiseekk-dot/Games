...
async function checkReleasesNow(games){ if(!("serviceWorker"in navigator))return; const reg=await navigator.serviceWorker.ready; if(!reg.active)return; reg.active.postMessage({type:"CHECK_RELEASES",games}); }
...
/* TABS — 5 tabs, smaller text */
.tabs{display:flex;gap:3px;background:${G.card};border:1px solid ${G.bdr};border-radius:13px;padding:4px}
.tab{flex:1;min-height:42px;padding:6px 2px;border:none;border-radius:9px;background:transparent;color:${G.dim};font-family:'Syne',sans-serif;font-size:9px;font-weight:600;cursor:pointer;white-space:nowrap;position:relative;line-height:1.3}
.tab.on{background:rgba(0,212,255,.15);color:${G.blu}}
.tab-dot{position:absolute;top:5px;right:5px;width:5px;height:5px;border-radius:50%;background:${G.org};animation:pulse 1.5s infinite}
...
/* ── HOME CARDS ── */
.hcard{background:${G.card};border:1px solid ${G.bdr};border-radius:16px;padding:16px;margin-bottom:12px;animation:fadeIn .3s ease;overflow:hidden;max-width:100%}
.hcard-hdr{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px}
.hcard-ico{font-size:16px;margin-right:6px}
.hcard-title{font-size:10px;font-weight:700;color:${G.dim};letter-spacing:.1em;text-transform:uppercase;display:flex;align-items:center}
...
.cont-title{font-size:15px;font-weight:700;margin-bottom:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.cont-meta{font-size:11px;color:${G.dim};margin-bottom:8px}
.prog-bar{height:4px;background:${G.bdr};border-radius:2px;overflow:hidden;margin-bottom:4px}
.prog-fill{height:100%;border-radius:2px;background:linear-gradient(90deg,${G.blu},${G.pur});transition:width .4s ease}
...
.rec-btn{margin-top:10px;padding:9px 16px;border:none;border-radius:9px;background:rgba(167,139,250,.15);color:${G.pur};font-family:'Syne',sans-serif;font-size:12px;font-weight:700;cursor:pointer;border:1px solid rgba(167,139,250,.3);width:100%}
.rec-btn:active{opacity:.75}

/* Countdown */
.cnt-big{font-family:'Orbitron',monospace;font-size:36px;font-weight:900;color:${G.org};line-height:1;margin-bottom:4px}
.cnt-label{font-size:11px;color:${G.dim};margin-bottom:12px}
.cnt-game-row{display:flex;gap:10px;align-items:center;margin-bottom:12px}
...
.cnt-name{font-size:13px;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.cnt-date{font-size:11px;color:${G.dim};margin-top:2px}
.cnt-actions{display:flex;gap:6px;flex-wrap:wrap}
.cnt-btn{padding:8px 12px;border-radius:9px;border:1px solid ${G.bdr};background:${G.card2};color:${G.txt};font-family:'Syne',sans-serif;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;flex:1;text-align:center}
.cnt-btn:active{opacity:.75}
.cnt-btn-primary{background:linear-gradient(135deg,${G.blu},#0060FF);color:#fff;border-color:transparent;box-shadow:0 0 12px rgba(0,212,255,.25)}
.cnt-btn-success{background:linear-gradient(135deg,${G.grn},#00a040);color:#000;border-color:transparent}

/* Finance insight */
.insight-row{display:flex;gap:8px;flex-wrap:wrap}
.insight-pill{padding:8px 12px;border-radius:10px;font-size:11px;font-weight:600;line-height:1.4;flex:1;min-width:140px}
...
/* UPCOMING CARDS */
.upc-card{width:100%;max-width:100%;background:${G.card};border:1px solid ${G.bdr};border-radius:16px;margin-bottom:12px;overflow:hidden}
...
.upc-banner-ov{position:absolute;inset:0;background:linear-gradient(to right,rgba(8,11,20,.92) 0%,rgba(8,11,20,.3) 100%)}
.upc-banner-title{position:absolute;bottom:8px;left:12px;font-size:14px;font-weight:700}
.upc-banner-days{position:absolute;top:8px;right:10px;font-family:'Orbitron',monospace;font-size:11px;font-weight:900;padding:4px 10px;border-radius:8px;background:rgba(255,159,28,.2);color:${G.org};border:1px solid rgba(255,159,28,.35)}
.upc-body{padding:10px 12px 12px}
.upc-date{font-size:11px;color:${G.dim};margin-bottom:10px}
.upc-actions{display:flex;gap:6px;flex-wrap:wrap}
...
.upc-btn-buy{border-color:rgba(0,212,255,.3);color:${G.blu};background:rgba(0,212,255,.07)}
...
.notif-toggle-row{display:flex;align-items:center;justify-content:space-between;padding-top:8px;margin-top:8px;border-top:1px solid ${G.bdr}}
.notif-toggle-lbl{font-size:11px;color:${G.dim}}
...
.section-title{font-family:'Orbitron',monospace;font-size:10px;font-weight:700;color:${G.dim};letter-spacing:.1em;text-transform:uppercase}
...
.fin-kv{font-family:'Orbitron',monospace;font-size:16px;font-weight:900;color:var(--c);line-height:1;margin-bottom:4px}
.fin-kl{font-size:9px;color:${G.dim};font-weight:600;letter-spacing:.07em;text-transform:uppercase}

/* INSIGHT CARDS */
.ins-card{border-radius:13px;padding:14px;margin-bottom:10px;border:1px solid transparent;animation:fadeIn .3s ease}
.ins-ico{font-size:22px;margin-bottom:8px}
.ins-title{font-size:12px;font-weight:700;margin-bottom:4px}
.ins-body{font-size:11px;line-height:1.6;opacity:.85}
.ins-val{font-family:'Orbitron',monospace;font-size:18px;font-weight:900;margin-top:8px}
...
.ntgl-lbl{font-size:14px;color:${G.txt}}.ntgl-sub{font-size:10px;color:${G.dim};margin-top:2px}
.ntgl-sw2{width:44px;height:26px;border-radius:13px;background:${G.bdr};position:relative;flex-shrink:0;transition:background .2s}
.ntgl-sw2.on{background:${G.blu}}
.ntgl-knob2{position:absolute;top:3px;left:3px;width:20px;height:20px;border-radius:50%;background:#fff;transition:transform .2s}
.ntgl-sw2.on .ntgl-knob2{transform:translateX(18px)}
  ...
  return(
    <div className="ovr">
      <div className="mdl">
        ...
        <div className="fin-divider"/><div className="fin-section-lbl">💰 Finanse</div>
        <div className="fin-row">
          ...
          <div className="fg"><label className="fl">Sklep / źródło</label>
        ...
        <div className="sold-toggle" onClick={()=>upd("priceSold",f.priceSold==null?"":null)}>
          ...
          <div className={"sold-sw"+(f.priceSold!=null?" on":"")}><div className="sold-knob"/></div>
        </div>
        ...
        {f.releaseDate&&<div className="fg"><label className="fl">Powiadomienia o premierze</label>
          {notifPermission==="denied"?<div style={{fontSize:11,color:G.red,padding:"8px 0"}}>⛔ Powiadomienia zablokowane</div>:(
            <div className="ntgl" onClick={async()=>{ if(!f.notifyEnabled&&notifPermission!=="granted")await onRequestNotif(); upd("notifyEnabled",!f.notifyEnabled); }}>
              <div><div className="ntgl-lbl">🔔 Powiadamiaj o premierze</div><div className="ntgl-sub">3 dni przed i w dniu premiery</div></div>
              <div className={"ntgl-sw2"+(f.notifyEnabled?" on":"")}><div className="ntgl-knob2"/></div>
            </div>
          )}
        </div>}
        <div className="mac">
...
// ── HOME ──────────────────────────────────────────────────────────────────────
function Home({games, onOpen, onStatusChange}){
  ...
  // Current game — pick highest hours or first
  ...
  const remHrs = active&&active.targetHours>0 ? Math.max(0,active.targetHours-active.hours) : null;

  // Recommendation: highest rated unplayed genre match, or longest in backlog
  const rec = (() => {
    if(!backlog.length)return null;
    const favGenre = games.filter(g=>g.status==="ukonczone"&&g.genre&&g.rating>=8).sort((a,b)=>b.rating-a.rating)[0]?.genre;
    const genreMatch = favGenre&&backlog.find(g=>g.genre===favGenre);
    return genreMatch||backlog[0];
  })();

  // Finance insight
  const totalSpent  = bought.reduce((s,g)=>s+ +g.priceBought,0);
  const totalEarned = sold.reduce((s,g)=>s+ +g.priceSold,0);
  const sellable    = games.filter(g=>g.status==="porzucone"&&!!+g.priceBought&&g.priceSold==null).sort((a,b)=>+b.priceBought - +a.priceBought);
  ...
  return (
    <div className="scr">
      ...
      <div style={{marginBottom:16}}>
        <div style={{fontFamily:"'Orbitron',monospace",fontSize:13,fontWeight:700,color:G.blu,letterSpacing:".06em",marginBottom:2}}>
          {new Date().getHours()<12?"🌅 Dzień dobry":"🎮 Cześć!"}
        </div>
        <div style={{fontSize:11,color:G.dim}}>
          {games.length} gier w kolekcji · {current.length} aktywnych
        </div>
      </div>
      ...
      {active ? (
        <div className="hcard" onClick={()=>onOpen(active)} style={{cursor:"pointer"}}>
          <div className="hcard-hdr">
            <span className="hcard-title"><span className="hcard-ico">▶️</span>Kontynuuj granie</span>
            <span className="hcard-badge" style={{background:"rgba(0,212,255,.12)",color:G.blu}}>GRAM</span>
          </div>
          <div className="cont-game">
            {active.cover
              ?<div className="cont-cover" style={{backgroundImage:`url(${active.cover})`}}/>
              :<div className="cont-cover0">{active.abbr||"??"}</div>}
            <div className="cont-body">
              <div className="cont-title">{active.title}</div>
              <div className="cont-meta">{active.genre||""}{active.genre&&active.hours?" · ":""}{active.hours?`${active.hours}h zagranych`:""}</div>
              {prog!==null&&<>
                <div className="prog-bar"><div className="prog-fill" style={{width:prog+"%"}}/></div>
                <div className="prog-label"><span>{prog}%</span><span>{remHrs}h pozostało</span></div>
              </>}
              {prog===null&&active.hours>0&&<div style={{fontSize:11,color:G.dim}}>Dodaj cel godzinowy żeby śledzić postęp</div>}
            </div>
          </div>
        ...
        <div className="hcard">
          <div className="hcard-hdr"><span className="hcard-title"><span className="hcard-ico">▶️</span>Kontynuuj granie</span></div>
          <div style={{textAlign:"center",padding:"16px 0",color:G.dim,fontSize:12}}>Nie grasz teraz w żadną grę.<br/>Zmień status gry na „Gram".</div>
        </div>
      )}
      ...
      {rec&&(
        <div className="hcard">
          <div className="hcard-hdr">
            <span className="hcard-title"><span className="hcard-ico">🎯</span>Co teraz grać</span>
          ...
          <button type="button" className="rec-btn" onClick={e=>{e.stopPropagation();onStatusChange(rec.id,"gram");}}>▶ Zacznij grać</button>
        </div>
      )}

      {/* 3 — NAJBLIŻSZA PREMIERA */}
      {nextUp&&(
        <div className="hcard">
          <div className="hcard-hdr">
            <span className="hcard-title"><span className="hcard-ico">📅</span>Najbliższa premiera</span>
            {days===0&&<span className="hcard-badge" style={{background:"rgba(57,255,110,.12)",color:G.grn,animation:"pulse 1s infinite"}}>DZIŚ!</span>}
            {days>0&&<span className="hcard-badge" style={{background:"rgba(255,159,28,.12)",color:G.org}}>{days}d</span>}
          </div>
          <div className="cnt-game-row">
            {nextUp.cover?<div className="cnt-cover" style={{backgroundImage:`url(${nextUp.cover})`}}/>:<div style={{width:44,height:44,borderRadius:8,background:G.card2,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontFamily:"'Orbitron',monospace",fontSize:11,fontWeight:900,color:G.pur}}>{nextUp.abbr||"??"}</div>}
            <div className="cnt-info">
              <div className="cnt-name">{nextUp.title}</div>
              <div className="cnt-date">{days===0?"Premiera dzisiaj!":days===1?"Jutro!":fmtDate(nextUp.releaseDate)}</div>
            </div>
          </div>
          ...
          <div className="cnt-actions">
            {days>0?(
              <>
                <button type="button" className="cnt-btn upc-btn-plan" onClick={()=>onOpen(nextUp)}>📋 Szczegóły</button>
                <button type="button" className="cnt-btn upc-btn-buy" onClick={()=>onStatusChange(nextUp.id,"planuje")}>🛒 Przypomnij</button>
              </>
            ):(
              <>
                <button type="button" className="cnt-btn cnt-btn-success" onClick={()=>onStatusChange(nextUp.id,"gram")}>▶ Zacznij grać</button>
                <button type="button" className="cnt-btn cnt-btn-primary" onClick={()=>onOpen(nextUp)}>+ Do kolekcji</button>
              </>
            )}
          </div>
        </div>
      ...
      {bought.length>0&&(
        <div className="hcard">
          <div className="hcard-hdr">
            <span className="hcard-title"><span className="hcard-ico">💰</span>Finansowy insight</span>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{height:1,background:G.bdr,margin:"4px 0"}}/>
            ...
            {sellable.length>0&&(
              <div style={{marginTop:8,padding:"10px 12px",background:"rgba(57,255,110,.07)",border:"1px solid rgba(57,255,110,.2)",borderRadius:10,fontSize:11,color:G.txt,lineHeight:1.5}}>
                💡 Sprzedaj <strong>{sellable[0].title}</strong> i odzyskaj ~{pln(+sellable[0].priceBought * 0.6)}
                {sellable.length>1&&` (+${sellable.length-1} innych porzuconych)`}
              </div>
            )}
          </div>
        </div>
      ...
      {!active&&!rec&&!nextUp&&bought.length===0&&(
        <div className="empty"><div className="eic">🎮</div><div className="ett">Witaj w PS5 Vault</div><div className="ess">Naciśnij + żeby dodać pierwszą grę do kolekcji</div></div>
      )}
    </div>
  );
}
...
// ── UPCOMING ──────────────────────────────────────────────────────────────────
function Upcoming({games,onOpen,onToggleNotify,onStatusChange,notifPermission,onRequestNotif}){
  const upcoming = games.filter(g=>g.releaseDate&&daysUntil(g.releaseDate)>=0).sort((a,b)=>new Date(a.releaseDate)-new Date(b.releaseDate));
    ...
    <div className="scr">
      {notifPermission==="default"&&(
        <div className="notif-banner">
          ...
          <div className="notif-banner-txt">Włącz powiadomienia — alert 3 dni przed premierą i w dniu premiery</div>
          <button className="notif-banner-btn" onClick={onRequestNotif}>Włącz</button>
        </div>
      )}

      {upcoming.length>0&&<>
        <div className="section-hdr"><span className="section-title">Nadchodzące premiery</span><span className="section-count">{upcoming.length}</span></div>
        {upcoming.map(g=>{
          ...
          return(
            <div key={g.id} className="upc-card">
              <div className="upc-banner" style={g.cover?{backgroundImage:`url(${g.cover})`}:{}}>
                <div className="upc-banner-ov"/>
                <div className="upc-banner-title">{g.title}</div>
                {days===0?<div className="upc-banner-days" style={{color:G.grn,background:"rgba(57,255,110,.2)",borderColor:"rgba(57,255,110,.35)"}}>DZIŚ!</div>:days<=3?<div className="upc-banner-days">{days}d</div>:<div className="upc-banner-days">{days} dni</div>}
              </div>
                ...
                <div className="upc-actions">
                  {days===0?(
                    <>
                      ...
                      <button type="button" className="upc-btn upc-btn-add" onClick={()=>onOpen(g)}>+ Kolekcja</button>
                    </>
                  ):(
                    <>
                      <button type="button" className="upc-btn upc-btn-plan" onClick={()=>onOpen(g)}>📋 Edytuj</button>
                      <button type="button" className="upc-btn upc-btn-watch" onClick={()=>{}}>👀 Obserwuj</button>
                      <button type="button" className="upc-btn upc-btn-buy" onClick={()=>{}}>🛒 Kup</button>
                    </>
                  )}
                </div>
                <div className="notif-toggle-row" onClick={e=>e.stopPropagation()}>
                  <span className="notif-toggle-lbl">🔔 Powiadomienie o premierze</span>
                  <div className={"ntgl-sw"+(g.notifyEnabled?" on":"")} onClick={async()=>{ if(!g.notifyEnabled&&notifPermission!=="granted")await onRequestNotif(); onToggleNotify(g.id); }}>
      ...
      {released.length>0&&<>
        <div className="section-hdr" style={{marginTop:16}}><span className="section-title">📦 Już dostępne — dodaj do kolekcji</span><span className="section-count">{released.length}</span></div>
        {released.map(g=>(
          <div key={g.id} className="upc-card">
            <div className="upc-banner" style={g.cover?{backgroundImage:`url(${g.cover})`}:{}}>
              <div className="upc-banner-ov"/>
              <div className="upc-banner-title">{g.title}</div>
              <div className="upc-banner-days" style={{color:G.grn,background:"rgba(57,255,110,.2)",borderColor:"rgba(57,255,110,.35)"}}>✓ Wyszło</div>
            </div>
            <div className="upc-body">
      ...
      {tba.length>0&&<>
        <div className="section-hdr" style={{marginTop:16}}><span className="section-title">TBA — brak daty</span><span className="section-count">{tba.length}</span></div>
      ...
      {!upcoming.length&&!released.length&&!tba.length&&<div className="empty"><div className="eic">📅</div><div className="ett">Brak nadchodzących premier</div><div className="ess">Dodaj daty premier do gier ze statusem „Planuję"</div></div>}
    </div>
  );
}
...
function Stats({games}){
  ...
  const finKpis=[{l:"Wydano",v:pln(totalSpent),c:G.red,bg:"rgba(255,77,109,.07)"},{l:"Odzyskano",v:pln(totalEarned),c:G.grn,bg:"rgba(57,255,110,.07)"},{l:"Realny koszt",v:pln(netCost),c:netCost>0?G.org:G.grn,bg:"rgba(255,159,28,.07)"},{l:"Koszt/godzinę",v:costPerHr?costPerHr.toFixed(1)+" zł":"—",c:G.blu,bg:"rgba(0,212,255,.07)"}];

  // Insights
  const insights=[];
  if(bought.length){
    const worst=soldGames.filter(g=>g.roi<0).slice(-1)[0];
    const best=soldGames.filter(g=>g.roi>0)[0];
    const worstCph=[...withHours].sort((a,b)=>(+b.priceBought/b.hours)-(+a.priceBought/a.hours))[0];
    const bestCph=[...withHours].sort((a,b)=>(+a.priceBought/a.hours)-(+b.priceBought/b.hours))[0];
    ...
    if(worstCph&&worstCph.hours>0)insights.push({ico:"⚠️",color:G.org,bg:"rgba(255,159,28,.07)",title:"Najdroższe godziny",body:`${worstCph.title} — ${(+worstCph.priceBought/worstCph.hours).toFixed(1)} zł/h. Gry powyżej 10 zł/h są najmniej opłacalne.`,val:(+worstCph.priceBought/worstCph.hours).toFixed(1)+" zł/h"});
    if(bestCph&&bestCph.hours>0)insights.push({ico:"💎",color:G.blu,bg:"rgba(0,212,255,.07)",title:"Najlepsza wartość",body:`${bestCph.title} — tylko ${(+bestCph.priceBought/bestCph.hours).toFixed(1)} zł/h. To Twoja najlepsza inwestycja per godzina.`,val:(+bestCph.priceBought/bestCph.hours).toFixed(1)+" zł/h"});
    if(totalSpent>0)insights.push({ico:"💰",color:G.pur,bg:"rgba(167,139,250,.07)",title:"Podsumowanie finansowe",body:`Wydałeś łącznie ${pln(totalSpent)}, odzyskałeś ${pln(totalEarned)}. Realny koszt kolekcji to ${pln(netCost)}.`,val:pln(netCost)});
  }

  return(
    <div className="scr">
      <div style={{display:"flex",gap:3,background:G.card,border:`1px solid ${G.bdr}`,borderRadius:11,padding:4,marginBottom:14}}>
        {[["general","🎮 Ogólne"],["finance","💰 Finanse"],["insights","💡 Analiza"]].map(([k,l])=>(
          <button key={k} type="button" onClick={()=>setTab(k)} style={{flex:1,minHeight:40,padding:"7px 2px",border:"none",borderRadius:8,background:tab===k?"rgba(0,212,255,.15)":"transparent",color:tab===k?G.blu:G.dim,fontFamily:"'Syne',sans-serif",fontSize:10,fontWeight:600,cursor:"pointer"}}>
            {l}
          </button>
        ))}
      </div>
      ...
      {tab==="finance"&&<>
        {!bought.length?<div className="empty"><div className="eic">💰</div><div className="ett">Brak danych finansowych</div><div className="ess">Dodaj ceny kupna do gier</div></div>:<>
          <div className="fin-kgd">{finKpis.map(k=><div key={k.l} className="fin-kcd" style={{"--c":k.c,background:k.bg}}><div className="fin-kv">{k.v}</div><div className="fin-kl">{k.l}</div></div>)}</div>
          ...
          {soldGames.length>0&&<div className="ccd"><div className="ctl">📈 ROI sprzedanych gier</div>
            <ul className="top-list">{soldGames.map(g=><li key={g.id} className="top-item"><span className="top-title">{g.title}</span><span style={{fontSize:10,color:G.dim,flexShrink:0}}>{(+g.priceBought).toFixed(0)}→{(+g.priceSold).toFixed(0)} zł</span><span className={"top-val "+(g.roi>=0?"roi-pos":"roi-neg")}>{g.roi>=0?"+":""}{g.roi.toFixed(0)} zł</span></li>)}</ul>
          </div>}
          <div className="ccd"><div className="ctl">💸 Najdroższe gry</div>
            <ul className="top-list">{[...bought].sort((a,b)=>+b.priceBought - +a.priceBought).slice(0,5).map(g=><li key={g.id} className="top-item"><span className="top-title">{g.title}</span>{g.storeBought&&<span style={{fontSize:10,color:G.dim,flexShrink:0}}>{g.storeBought}</span>}<span className="top-val" style={{color:G.org}}>{(+g.priceBought).toFixed(0)} zł</span></li>)}</ul>
          </div>
          {withHours.length>0&&<div className="ccd"><div className="ctl">⏱ Najlepsza wartość (zł/h)</div>
            <ul className="top-list">{[...withHours].sort((a,b)=>(+a.priceBought/a.hours)-(+b.priceBought/b.hours)).slice(0,5).map(g=><li key={g.id} className="top-item"><span className="top-title">{g.title}</span><span style={{fontSize:10,color:G.dim,flexShrink:0}}>{g.hours}h</span><span className="top-val" style={{color:G.grn}}>{(+g.priceBought/g.hours).toFixed(1)} zł/h</span></li>)}</ul>
          </div>}
        </>}
      </>}

      {tab==="insights"&&<>
        {!insights.length?<div className="empty"><div className="eic">💡</div><div className="ett">Za mało danych</div><div className="ess">Dodaj ceny i godziny do gier żeby zobaczyć analizę</div></div>:
          insights.map((ins,i)=>(
            <div key={i} className="ins-card" style={{background:ins.bg,border:`1px solid ${ins.color}30`}}>
              <div className="ins-ico">{ins.ico}</div>
              <div className="ins-title" style={{color:ins.color}}>{ins.title}</div>
              <div className="ins-body">{ins.body}</div>
              <div className="ins-val" style={{color:ins.color}}>{ins.val}</div>
            </div>
          ))
        }
      </>}
...
export default function App(){
  ...
  const [notifPermission,setNotif] = useState(()=>"Notification"in window?Notification.permission:"denied");

  useEffect(()=>{ lsWrite(games); },[games]);
  useEffect(()=>{ registerSW().then(()=>{ const g=games.filter(g=>g.notifyEnabled&&g.releaseDate); if(g.length&&Notification.permission==="granted")checkReleasesNow(g); }); },[]);// eslint-disable-line

  const flash = msg=>{ setToast(msg); setTimeout(()=>setToast(null),1800); };
  const requestNotif = async()=>{ const p=await requestNotifPermission(); setNotif(p); return p; };
  ...
  function handleDel(id){ setGames(prev=>prev.filter(g=>g.id!==id)); setModal(null); flash("Usunięto"); }
  function toggleNotify(id){ setGames(prev=>prev.map(g=>g.id===id?{...g,notifyEnabled:!g.notifyEnabled}:g)); }
  function handleStatusChange(id,status){ setGames(prev=>prev.map(g=>g.id===id?{...g,status}:g)); flash(`Status zmieniony na ${SM[status]?.label}`); }
  ...
  return(
    <>
      ...
      <div className="app">
        <div className="hdr">
          ...
          <div className="tabs">
            ...
            <button type="button" className={"tab"+(tab==="upc"?" on":"")} onClick={()=>setTab("upc")} style={{position:"relative"}}>
              📅 Premiery{upcomingCount>0&&<span className="tab-dot"/>}
            </button>
            <button type="button" className={"tab"+(tab==="st"?" on":"")} onClick={()=>setTab("st")}>📊 Statsy</button>
          </div>
        </div>

        {tab==="home"&&<Home games={games} onOpen={setModal} onStatusChange={handleStatusChange}/>}
        ...
        {tab==="col"&&<>
          ...
          <div className="lst">
            {visible.length===0
              ...
              :visible.map(g=>{const m=SM[g.status]||SM.planuje;return(
                <div key={g.id} className="gc" style={{"--c":m.c,"--bg":m.bg}} onClick={()=>setModal(g)}>
                  ...
                  <div className="gcnt">
                    ...
                    <div className="grt">
                      ...
                      {!!+g.priceBought&&<span className={g.priceSold!=null?"gprice-sold":"gprice"}>{g.priceSold!=null?`${(+(g.priceSold||0) - +(g.priceBought||0))>=0?"+":""}${(+(g.priceSold||0) - +(g.priceBought||0)).toFixed(0)} zł`:`${(+g.priceBought).toFixed(0)} zł`}</span>}
                    </div>
                  </div>
                </div>
              );})}
        ...
        {tab==="upc"&&<Upcoming games={games} onOpen={setModal} onToggleNotify={toggleNotify} onStatusChange={handleStatusChange} notifPermission={notifPermission} onRequestNotif={requestNotif}/>}
        {tab==="st"&&<Stats games={games}/>}

        {modal&&<Modal game={modal==="add"?null:modal} onSave={handleSave} onDel={handleDel} onClose={()=>setModal(null)} notifPermission={notifPermission} onRequestNotif={requestNotif}/>}
